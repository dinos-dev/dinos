...
export interface ApiParams<PathParams = Record<string, unknown>, QueryParams = Record<string, unknown>, Body = unknown> {
...
const generateQueryString = (queryParams?: Record<string, unknown>): string => (queryParams ? `?${new URLSearchParams(Object.entries(queryParams).map(([k, v]) => [k, String(v)])).toString()}` : '')
